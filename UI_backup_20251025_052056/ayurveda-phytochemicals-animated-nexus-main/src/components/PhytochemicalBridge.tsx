...
import { apiService, DemoResponse } from "@/lib/api";
...
export default function PhytochemicalBridge() {
  ...
  useEffect(() => {
    const fetchDemoData = async () => {
      try {
        const data = await apiService.getDemoData();
        setDemoData(data);
      } catch (error) {
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {loading ? (
            <div className="col-span-full text-center py-12">
              ...
              <p className="mt-4 text-muted-foreground">Loading herb data...</p>
            </div>
          ) : demoData ? (
            demoData.herbs.map((herb, index) => {
              const properties = demoData.ayurvedic_properties[herb] || {};
              const compounds = demoData.modern_compounds[herb] || {};
                      ...
                      <div className="relative z-10 p-6">
                          ...
                          <div className="flex items-center gap-3 mb-3">
                            ...
                            <div>
                              <h4 className="text-xl font-bold text-accent">
                                {compoundNames.length > 0 ? compoundNames[0] : 'Unknown Compound'}
                              </h4>
                        ...
                        >
                          <p className="text-sm font-semibold text-center">
                            {properties.rasa ? `Rasa: ${properties.rasa.join(', ')}` : 'Ayurvedic Properties'}
                          </p>
                        </motion.div>
                      </div>
                    </Card>
